...
class PrologGameConverterSuite extends AnyFunSuite {
  ...
  test("Convert sequence of cards in string") {
    ...
    val sequence: Seq[Card] = Seq(Cards().ACE_CLUBS, Cards().FOUR_SPADES, Cards().KING_DIAMONDS)
  ...
  test("Convert a sequence of term into a tuple sequence") {
    ...
    val sequence: Seq[Card] = Seq(Cards().ACE_CLUBS, Cards().ACE_SPADES, Cards().ACE_DIAMONDS)
  ...
  test("Change optional value of the card Ace if it is after King card") {
    ...
    assertResult(Seq(Cards().QUEEN_CLUBS, Cards().KING_CLUBS, Cards().ACE_CLUBS.copy(rank = "14")))(prologConverter.optionalValueCards(Seq(Cards().QUEEN_CLUBS, Cards().KING_CLUBS, Cards().ACE_CLUBS)))

    assertResult(Seq(Cards().QUEEN_CLUBS, Cards().KING_CLUBS, Cards().ACE_CLUBS.copy(rank = "14"), Cards().TWO_CLUBS, Cards().THREE_CLUBS))(prologConverter.optionalValueCards(Seq(Cards().QUEEN_CLUBS, Cards().KING_CLUBS, Cards().ACE_CLUBS, Cards().TWO_CLUBS, Cards().THREE_CLUBS)))

    assertResult(Seq(Cards().ACE_CLUBS, Cards().TWO_CLUBS, Cards().THREE_CLUBS))(prologConverter.optionalValueCards(Seq(Cards().ACE_CLUBS, Cards().TWO_CLUBS, Cards().THREE_CLUBS)))
  }

  test("Convert a tuple sequence into string") {

    val tupleSeq: Seq[(Int, String)] = Seq(Cards().QUEEN_CLUBS, Cards().KING_CLUBS) map (card => (card.rank.value, card.suit.name))
    val varX = new Var("X")
