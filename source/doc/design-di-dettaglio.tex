...
\end{itemize}
L’object CardUtils è invece una utility che permette di ottenere il percorso dell’immagine raffigurante la carta da gioco a partire dall’entità Card.
...
La parte di view si compone di 4 scene che vengono caricate all’interno di \textit{StartupStage}.
...
\end{itemize}
Alla selezione di una di queste modalità, viene caricata una nuova scena, alla quale viene passato un listener di tipo \textit{StartUpSceneListener} che tramite callback permette di reagire alle azione di submit() e della pressione del tasto back.
\newline
Ciascuna scena scena eredita da \textit{StartupBaseScene} e si compone di 3 parti:
\begin{itemize}
...
I risultati delle varie operazioni vengono poi notificati al controller attraverso la funzione notifyEvent richiesta in costruzione, a cui viene passato un oggetto di tipo GameStartupEvent.
...
In seguito ad ogni mossa effettuata dal giocatore durante il turno, la History viene aggiornata e utilizzata per ripristinare lo stato delle versioni precedenti a seguito delle azioni di undo/redo.
...
\end{center}
%todo: Here is missing the diagram. Was the same as next paragraph.
